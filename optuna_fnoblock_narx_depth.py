...
def save_data(Train_L2, Test_L2, mode1, width, depth):
    ...
    path = 'D:/LYK/gzb2yunchi/FNO/optuna/h/'+trial
    os.makedirs(path)
...
def objective(trial):
    ...
    torch.save({'model':model.state_dict()}, path +'/h.pth')
    return test_l2
...
################################################################
# Initial Condition
h_reader = MatReader(H_DATA_PATH)
train_ini_h = h_reader.read_field('h')[:ntrain,::sub,::sub]
test_ini_h = h_reader.read_field('h')[ntrain:ntrain+ntest,::sub,::sub]

# Depth Label
label_reader = MatReader(H_LABEL_PATH)
train_label_h = label_reader.read_field('h')[1:ntrain+1,::sub,::sub]
test_label_h = label_reader.read_field('h')[ntrain+1:ntrain+ntest+1,::sub,::sub]

# Dimension expand
train_ini_h = train_ini_h.reshape(ntrain,S_x,S_y,1)
test_ini_h = test_ini_h.reshape(ntest,S_x,S_y,1)
train_label_h = train_label_h.reshape(ntrain,S_x,S_y,1)
test_label_h = test_label_h.reshape(ntest,S_x,S_y,1)
...
train_label_h = y_normalizer.encode(train_label_h)
...
bc_train = torch.cat((inflow_train, wl_train), dim=-1)
bc_test =  torch.cat((inflow_test, wl_test), dim=-1)
train_loader = torch.utils.data.DataLoader(torch.utils.data.TensorDataset(train_ini_h, bc_train, train_label_h), batch_size=batch_size, shuffle=True)
test_loader = torch.utils.data.DataLoader(torch.utils.data.TensorDataset(test_ini_h, bc_test, test_label_h), batch_size=batch_size2, shuffle=False)
...
study = optuna.create_study(study_name='depth',direction='minimize',storage='sqlite:///h.sqlite3', 
...
# optuna-dashboard sqlite:///h.sqlite3
######################################## 
